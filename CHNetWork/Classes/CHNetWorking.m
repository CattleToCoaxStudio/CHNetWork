...
{   //这里写的这个取消请求逻辑， 是另外一种写法， 就是在在外写创建实例，用实例方法来
    //做网络请求，当外面的网络请求还在进行中，  但外面的控制器以经dealloc了的话就会
    //来到这里取消请求
...
-(NSURLSessionDataTask *)ch_GetRequestWithDeleagteTarget:(id)delegate
    ...
    switch (type) {
        ...
        //无网络状态
        CHURLResponse *response = [[CHURLResponse alloc] initWithResponseString:@"" requestId:0 request:nil responseData:nil status:CHURLResponseErrorTypeNoNetWork];
        [self failOnCallingAPI:response withErrorType:CHURLResponseErrorTypeNoNetWork];
        return nil;
    }
...
-(NSURLSessionDataTask *)ch_UploadTaskWithDeleagteTarget:(id)delegate
    ...
    if ([self isReachable]) {
        ...
        //和网络菊花
        NSURLSessionDataTask *requestTask = nil;
        requestTask =  [[CHApiProxy shardInstance] callUploadFilewithParams:param success:^(CHURLResponse *response) {
            [self successedOnCallingAPI:response];
        } fail:^(CHURLResponse *response) {
            [self failOnCallingAPI:response withErrorType:CHURLResponseErrorTypeDefault];
        }];
            ...
            [self.delegate ch_requestCallAPISuccess:self];
                ...
                if ([self.delegate respondsToSelector:@selector(ch_requestCallAPISuccess:)]) {
                    ...
                    if ([object isKindOfClass:[NSArray class]]) {
                        ...
                        model = [self.modeClass mj_objectWithKeyValues:object];
                    }
                    self.model = model;
                    [self.delegate ch_requestCallAPISuccess:self];
                }
...
-(BOOL)hasCacheWithParams:(NSDictionary *)params
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CHURLResponse *response = [[CHURLResponse alloc]initWithData:data];
        [self successedOnCallingAPI:response];
...
-(BOOL)isReachable
{
    CHNetWorkStatus _Reachability = [CHReachability networkStatus];
    
    return _Reachability == CHNetWorkStatusNotReachable ? NO : YES;
